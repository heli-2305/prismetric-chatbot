...
@st.cache_resource
def load_model():
    model_path = os.path.join(BASE_DIR, "intent_classifier_logreg.joblib")
...
def get_response(intent):
    ...
    if intent == "ask_recommendation":
        return "I can help you choose the right service. Please describe your project goal or industry."
...
if prompt := st.chat_input("Type your message here..."):
    ...
    st.session_state.messages.append({"role": "assistant", "content": response})    probabilities = model.predict_proba([user_input])[0]
    classes       = model.classes_
    best_index    = probabilities.argmax()
    intent        = classes[best_index]
    confidence    = probabilities[best_index]
